...
def compile_template(temp, mod):
    ...
    if mod == '2':
        ...
        temp = delete_continuous_wildcard(temp)
        temp = temp.replace('\\', '\\\\')
        temp = (temp.replace("[", r"\[").replace("]", r"\]").replace("(", r"\(").replace(")", r"\)").replace("{", r"\{").replace("}", r"\}")
                  .replace("$", r"\$").replace("+", r"\+").replace("?", r"\?")).replace("|", r"\|").replace(".", r"\.")
        # \가 추가된 <\*> 처리 완료
        temp = temp.replace("*", r"\*")
        temp = temp.replace(r"<\*>", "(.*)")
...
def get_variance_info(cur_log, cur_temp):
    if "PrivilegedAction" in cur_log:
        match = None
    else:
        # print(cur_temp)
...
if __name__ == '__main__':
    for parser in parsers:
        ...
        for line in tqdm(rdr):
            ...
            # log = " ".join(line[content_idx].strip().split())
            # template = " ".join(line[template_idx].strip().split())
            ...
            # match가 None이라서 그 로그와 템플릿은 스킵해야함. 왜 None이 뜬지는 확인
            if cur_variable_list == -1 and cur_char_set == -1:
                match_is_None_drc += (len(log) * math.ceil(math.log2(len(set(log)))))
                continue
        ...
        ## match안되는건 그냥 .*로 치부
        print("match가 None: ", match_is_None_drc)
        total_drc += match_is_None_drc
