    ...
    def process(self, data_bundle: DataBundle) -> DataBundle:
        ...
        def get_new_ins(bpes, spans, indexes):
            if len(bpes)>512:
                ...
                indexes = indexes[:100]
                # print(spans)
                # decoded_text = self.tokenizer.decode(bpes, skip_special_tokens=True)
                # print("bpe text: ", decoded_text)
        ...
        def process(ins):
            ...
            for s, e, t in raw_ents:
                ## s= -1 e+1 = 22 인 경우로 old와 new_ent_str이 길이가 같지않음
                if len(raw_words[s:e+1]) and s>0:
            ...
            for _raw_words, _raw_ents in zip(raw_sents, raw_entss):
                ...
                for idx, word in enumerate(_raw_words, start=0):
                    ...
                    ## word의 길이 자체가 3000이 넘은 그런 단어는 토큰화해도 이미 512를 넘겨서 encode 메소드가 경고문을 남김
                    else:
                        if len(word) > 512:
                            word = word[:512]
            ...
            ## 길이가 같지 않을 때가 있는데 보통 old가 문제됨
            # print("new: ", new_ent_str)
            # print("old: ", old_ent_str)
