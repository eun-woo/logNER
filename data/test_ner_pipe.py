...
class SpanNerPipe(UnifyPipe):
    ...
    def process_from_file(self, input_json: list) -> DataBundle:
        dl = SpanLoader().loadFromList(input_json)
        return self.process(dl)


class SpanLoader(Loader):
    def loadFromList(self, data_lst):
        ds = DataSet()
        for data in data_lst:
            # 수정: Json이 아니라 dictionary가 들어오기에 주석처리함
    ...
    # def _loadFromList(self, data_lst):
    #     ds = DataSet()
