...
def decode(scores, length, allow_nested=False, thres=0.5):
    ...
    for idx, (curr_scores, curr_len) in enumerate(zip(scores, length.cpu().tolist())):
        ...
        tmp_scores = curr_scores[:curr_len, :curr_len][curr_non_mask].detach().cpu().numpy()  # -1 x 2  # model의 output으로 나온 tensor이기에 augrad 기능이 있음. detach()로 augograd정보 제거 후 numpy() 수행가능. 또한 gpu에 선언된 텐서인 경우, cpu()로 cpu에 선언 후, numpy()로 변환 가능
