...
class CNNNer(nn.Module):
    ...
    def forward(self, input_ids, bpe_len, indexes, matrix):
        # original_sentence = self.tokenizer.decode(input_ids[0], skip_special_tokens=True)
        ...
        state = scatter_max(last_hidden_states, index=indexes, dim=1)[0][:, 1:]  # bsz x word_len x hidden_size     # 각 단어 그룹별로 최대 vec 리턴턴
