...
def get_data(dataset_name, model_name):
    ...
    if dataset_name == 'ace2004':
        ...
        paths = 'preprocess/outputs/customized_data'
...
def filter_empty_strings(dataset):
    """빈 문자열을 제외한 DataSet을 반환"""
    return DataSet([item for item in dataset if item['word_len'] != 0])
...
# model = torch.load("/home/eunwoo/CNN_Nested_NER/model_cache/cassan_spark_hadoop680.pth")
# optimizer
parameters = []
ln_params = []
non_ln_params = []
...
torch.save(model, "/home/eunwoo/CNN_Nested_NER/model_cache/union.pth")
